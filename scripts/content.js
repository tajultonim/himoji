function observeAll() {
  var mutationObserver = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      ...
      let inpWrp = document.querySelector(".sc-cnOchM.bvArzR");
      if (inpWrp) {
        if (inpWrp.children.length == 2) {
          inpWrp.removeChild(inpWrp.lastChild);
          if (inpWrp.firstChild.disabled) {
            inpWrp.firstChild.disabled = false;
            inpWrp.firstChild.style.backgroundColor = "rgb(70, 77, 99)";
            inpWrp.firstChild.style.color = "white";
          }
        }
      }
    ...
    `;
      ...
      if (ewrp) {
        ...
        if (!ebtn) {
          ...
          nel.onclick = function () {
            if (confirm("সত্যি সত্যি ব্রেক করতে চান? 🥴") == true) {
              breakTable();
            }
          };
          nel.className = "sc-gvSjmx eSYusR";
          nel.style.color = "white";
          nel.innerHTML = `B`;
          ewrp.prepend(nel);
        }
      }
